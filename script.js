...
const state={envs:[],scope:{},sync:{},cache:{},serverCache:{},sessions:JSON.parse(sessionStorage.getItem('hlx.sessions')||'{}'),source:sessionStorage.getItem('hlx.sourceEnv')||'',target:sessionStorage.getItem('hlx.targetEnv')||'',type:'form',mode:'browse',objects:[],compare:null,selected:[],statusFilter:'',sortKey:'name',sortDir:'asc'};
...
async function refreshAll(){try{const e=await api('/api/environments');state.envs=e.environments;state.scope=e.scope||{};state.sync=e.sync||{};state.serverCache=e.serverCache||{};const c=await api('/api/cache/summary');state.cache=c.environments||{};renderSelects();renderTree();renderCards();renderJobs();renderScope();renderHeader();setStatus(state.serverCache.running?'Server sync is running...':'Ready')}catch(err){setStatus(err.message)}}
...
function renderCards(){envCards.innerHTML=state.envs.map(env=>{const s=state.serverCache.environments?.[env]||{};const counts=state.cache[env]||{};const rows=Object.entries(counts).sort().map(([t,v])=>`<div class="metric"><span>${TYPE_LABEL[t]||t}</span><span>${v.count} <span class="muted">${fmtDate(v.lastSeen)}</span></span></div>`).join('')||'<div class="muted">No cache yet</div>';return `<div class="env-card"><div class="toolbar"><h3>${env.toUpperCase()} <span class="${s.status==='ok'?'ok':s.status==='error'?'bad':'warn'}">${s.status||'pending'}</span></h3><button onclick="serverSyncEnv('${env}')">Server Sync</button></div><div class="small muted">Server: ${s.serverVersion||'-'}<br>User: ${s.user||'-'}<br>Last sync: ${fmtDate(s.finishedAt)}<br>Mode: ${s.lastSyncMode||'-'}</div><div class="small" style="margin:7px 0">Scope: ${(s.scope?.include_form_prefixes||state.scope.include_form_prefixes||[]).join(', ')||'-'}</div>${rows}</div>`}).join('')}
...
async function loadSourceObjects(){savePair();state.mode='browse';state.compare=null;state.statusFilter='';summary.innerHTML='';setStatus(`Loading ${TYPE_LABEL[state.type]} from ${state.source.toUpperCase()}...`);const q=encodeURIComponent(searchText.value||'');state.objects=(await api(`/api/objects/${state.type}?environment=${encodeURIComponent(state.source)}&q=${q}&limit=5000`)).objects||[];renderRows();renderHeader();setStatus(`${state.objects.length} object(s) shown from ${state.source.toUpperCase()}`)}
...
function toggleActivityLog(){const p=document.getElementById('activityPanel');const b=document.getElementById('activityToggleBtn');if(!p)return;p.classList.toggle('expanded');p.classList.toggle('collapsed');if(b)b.textContent=p.classList.contains('expanded')?'Collapse':'Expand';}
init();
